...
export default function StoreSearchPage() {
    ...
    const handleReservation = async (storeKey: number) => {
        ...
        try {
            const queue = `store-${storeKey}`;
            const userId = userInfo.id;

            // 1. 먼저 대기열에 등록
            await fetch(`${apiUrl}/api/v1/queue?queue=${queue}&user_id=${userId}`, {
                method: 'POST'
            });

            // 2. 진입 가능한 상태인지 확인
            const response = await fetch(`${apiUrl}/api/v1/queue/allowed?queue=${queue}&user_id=${userId}`);
            if (!response.ok) {
            ...
            if (result.allowed) {
                // 예약 페이지로 이동
                router.push(`/app/booking/page.tsx?storeKey=${storeKey}`);
            } else {
                // 대기열 페이지로 이동
                router.push(`/app/booking/loading.tsx?storeKey=${storeKey}`);
            }
